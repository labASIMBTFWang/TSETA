...
async function restore_rdna() {
	...
	fs.writeFileSync("output/rDNA_info.json", JSON.stringify(rDNA_info));

	if (DEBUG) {
		console.log("debug no output file");
	}
	else {
		let output_path = `output/mafft_ch${nChr}.fa`;
...
async function find_rDNA_use_blastn(rDNA_filePath, subject_genome_name, nChr) {
	const subject_chrInfo = loadChrLength(`output/${subject_genome_name}.length.txt`).list;
	const subject_chr_name = subject_chrInfo[nChr - 1].chr;

	const subject_fa_filename = `tmp/fasta/${subject_genome_name}_${subject_chr_name}.fa`;
	...
	if (!DEBUG) {
		fs.writeFileSync(
			`tmp/blastn_rDNA_${subject_chr_name}.txt`,
	...
	if (!(max_send > min_sstart)) {
		...
		let raw_fa = readFasta(`tmp/fasta/${subject_genome_name}_${subject_chr_name}.fa`)[subject_chr_name];	
