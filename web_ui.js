...
class ViewerState {
	...
	get ref1() {
		return dataset.refs[0];
	}
	get ref2() {
		return dataset.refs[1];
	}
	get nChr() {
		return this._nChr;
	}
...

{
	...
	el_input_chr.oninput = (evt) => {
		...
		const input_file = dataset.results[viewerState.nChr - 1];
...
async function loadData(input_file) {
	let raw_fa = await fetchData(input_file, "text");
...
function _render(options) {
	...
	for (let i = 0; i < viewerState._position_makers.length; ++i) {
		...
		ctx.beginPath();
		ctx.rect(x1, offset_y, bp_size, 0);
	...
	if (viewerState.ref1 && viewerState.ref2 && viewerState.nChr) {
		...
		dataset.forEach(({ refName, ref_mapto_ma }, sid) => {
			let gc_list = gc_content[refName][viewerState.nChr];
			if (!gc_list || gc_list.length <= 0) {
				return;
			}
			let row_y_bottom = 1 * (gc_max_height + seg_row_separate);

			let avg_top = row_y_bottom - gc_max_height;
			let avg_mid = row_y_bottom - gc_max_height * 0.5;
			let avg_bottom = row_y_bottom;
			...
			ctx.beginPath();
			ctx.moveTo(0, row_y_bottom - seg_row_separate);
			for (let i = 0; i < gc_list.length; ++i) {
				...
				if (x2 >= 0 && x1 <= max_view_width) {
					...
					let y = row_y_bottom - gc - 1;

					ctx.lineTo(x1, y);
					ctx.lineTo(x2, y);
				}
			}
			//ctx.lineTo(max_view_width, row_y_bottom - seg_row_separate);
			ctx.strokeStyle = "black";
			ctx.stroke();
